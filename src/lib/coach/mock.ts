// The stand-in coach.
//
// What the review route returned before Gemini was wired in, kept as
// the fallback for a deployment without a GEMINI_API_KEY (a preview, a
// laptop without the env pulled) so the practice loop still runs end
// to end. Deterministic per attempt, rewards repetition, and says so
...
export function mockReview(body: MockRequest): ReviewResponse | null {

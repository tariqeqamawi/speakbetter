...
- Students need to be able to watch back the video they submitted. If hosting that playback inside the app turns out to be cost-prohibitive, the fallback is to not import the video into the app at all — reference it directly from the student's own phone, so playback happens from their own device rather than from app-hosted storage.
...
- **The spread** is one card pulled at random from every color at once — seven ingredients for one talk, which is the whole argument of the deck in a single gesture. Deal it again and you have a different talk. Random on purpose: a hand you chose is a hand of what you already do. It's drawn as a hand: seven cards fanned over each other, face up, in the order the colors run, with the seven lessons named under the fan. Under a mouse a card lifts out of the fan and comes forward while it's pointed at — or while its name is — so it can be read where it lies.
